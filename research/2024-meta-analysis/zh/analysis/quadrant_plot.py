...
模块功能：
    基于 Pandas、Matplotlib 与 Seaborn，模拟多干预/算法在 Accuracy、Time、Safety、Resource
    四维指标上的性能数据，聚焦 Accuracy（越高越优）与 Time（越低越优）两核心要素，动态划分
    并可视化四象限分布，助力科研与企业决策。

核心特点：
  - 可复现数据模拟：支持定制化随机种子，自动校验并去重算法/干预名称列表；
  - 灵活阈值划分：自动或指定 Accuracy/Time 分界，四象限半透明背景区分显著对比；
  - 出版级美化：柔和主/次网格、虚线分隔、交点高亮、四象限中心标签、双色散点与描边注释；
  - 企业级稳定性：中文结构化日志、全面异常捕获、全局样式管理，满足线上部署与科研复现；
  - 多模式输出：支持交互式展示与高分辨率图像保存。
